...
var WorldTracking_XR8 = class extends TrackingMode {
   ...
   */
  _extraPermissions = [];
  onTrackingStatus = new Emitter2();
  onImageScanning = new Emitter2();
  onImageFound = new Emitter2();
  onImageUpdate = new Emitter2();
  onImageLost = new Emitter2();
  onMeshFound = new Emitter2();
  onWaySpotFound = new Emitter2();
  onWaySpotUpdated = new Emitter2();
  onWaySpotLost = new Emitter2();
  /**
...
var FaceTracking_XR8 = class extends TrackingMode {
  ...
  /* Cache 8th Wall cam rotation */
  _cachedRotation = [0, 0, 0, -1];
  onFaceScanning = new Emitter2();
  onFaceLoading = new Emitter2();
  onFaceFound = new Emitter2();
  onFaceUpdate = new Emitter2();
  onFaceLost = new Emitter2();
  /** Consumed by 8th Wall. */
  listeners = [
    {
      /**
...
var simd3 = async () => WebAssembly.validate(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 5, 1, 96, 0, 1, 123, 3, 2, 1, 0, 10, 10, 1, 8, 0, 65, 0, 253, 15, 253, 98, 11]));
var threads3 = () => (async (e) => {
  try {
...
var RetainEmitter3 = class extends Emitter3 {
  /** Pre-resolved data. @hidden */
...
var _CollisionComponent3 = class extends Component3 {
   ...
   */
  set radius(radius) {
    const length2 = this.collider === Collider3.Sphere ? radius : 2 * radius / SQRT_3;
...
var Material3 = class {
   ...
   */
  static wrap(engine2, index) {
    return index > 0 ? new Material3(engine2, index) : null;
  }
};
var temp2d = null;
var Texture3 = class {
...
var Object3D3 = class {
  ...
  getComponent(typeOrClass, index = 0) {
    const type = isString3(typeOrClass) ? typeOrClass : typeOrClass.TypeName;
    const wasm = this._engine.wasm;
  ...
  getComponents(typeOrClass) {
    ...
    let type = null;
    if (typeOrClass) {
      type = isString3(typeOrClass) ? typeOrClass : typeOrClass.TypeName;
...
var I18N3 = class {
   ...
   * ```
   */
  onLanguageChanged = new Emitter3();
  /** Wonderland Engine instance. @hidden */
  _engine;
...
function fetchWithProgress3(path, onProgress) {
  return new Promise((resolve, reject) => {
...
var Scene3 = class {
  ...
  onPreRender = new Emitter3();
  /** Called after the scene has been rendered */
  onPostRender = new Emitter3();
  /** Wonderland Engine instance. @hidden */
  _engine;
  ...
  get activeViews() {
    ...
    for (let i = 0; i < count; ++i) {
      views.push(new ViewComponent3(this._engine, viewTypeIndex, this._engine.wasm._tempMemInt[i]));
    }
    return views;
  }
  /**
  ...
  async load(filename) {
    ...
    const wasm = this._engine.wasm;
    const buffer = await fetchWithProgress3(filename, (bytes, size2) => {
  ...
  async append(file, options = {}) {
    const { loadGltfExtensions = false, baseURL = isString3(file) ? getBaseUrl(file) : this._baseURL } = options;
    const wasm = this._engine.wasm;
    const buffer = isString3(file) ? await fetchWithProgress3(file) : file;
    let error = null;
    let result = void 0;
...
var TextureManager3 = class {
  ...
  load(filename, crossOrigin) {
    ...
    return new Promise((resolve, reject) => {
      image.onload = () => {
        let texture = new Texture3(this._engine, image);
        if (!texture.valid) {
   ...
   */
  wrap(id) {
    const texture = this.#cache[id] ?? (this.#cache[id] = new Texture3(this._engine, id));
    texture["_id"] = id;
    return texture;
  }
...
var WonderlandEngine3 = class {
   ...
   * ```
   */
  onXRSessionStart = new RetainEmitter3();
  /**
  ...
  #textures = new TextureManager3(this);
  /**
  ...
  isRegistered(typeOrClass) {
    return this.#wasm.isRegistered(isString3(typeOrClass) ? typeOrClass : typeOrClass.TypeName);
  }
  /**
  ...
  wrapObject(objectId) {
    ...
    const o = cache[objectId] || (cache[objectId] = new Object3D3(this, objectId));
  ...
  _wrapComponent(type, componentType, componentId) {
    ...
    let component;
    if (type == "collision") {
      component = new CollisionComponent3(this, componentType, componentId);
    } else if (type == "text") {
      component = new TextComponent3(this, componentType, componentId);
    } else if (type == "view") {
      component = new ViewComponent3(this, componentType, componentId);
    } else if (type == "mesh") {
      component = new MeshComponent3(this, componentType, componentId);
    } else if (type == "input") {
      component = new InputComponent3(this, componentType, componentId);
    } else if (type == "light") {
      component = new LightComponent3(this, componentType, componentId);
    } else if (type == "animation") {
      component = new AnimationComponent3(this, componentType, componentId);
    } else if (type == "physx") {
      component = new PhysXComponent3(this, componentType, componentId);
    } else {
...
function _setupDefaults(ctor) {
  for (const name in ctor.Properties) {
    ...
    if (p.type === Type3.Enum) {
      ...
      p.default = p.default ?? _componentDefaults3.get(p.type);
    }
...
var WASM3 = class {
  ...
  _registerComponentLegacy(typeName, params, object) {
    const ctor = class CustomComponent extends Component3 {
    };
  ...
  tempUTF8(str2, byteOffset = 0) {
    const strLen = this.lengthBytesUTF8(str2) + 1;
    this.requireTempMem(strLen + byteOffset);
    const ptr = this._tempMem + byteOffset;
    this.stringToUTF8(str2, ptr, strLen);
    return ptr;
  }
  /**
  ...
  _wljs_xr_session_start(mode) {
    if (this._engine.xr === null) {
      this._engine.xr = new XR3(this, mode);
  ...
  _wljs_xr_session_end() {
    ...
    if (startEmitter instanceof RetainEmitter3)
      startEmitter.reset();
  ...
  _wljs_set_component_param_mesh(c, p, pe, v) {
    const param = this.UTF8ViewToString(p, pe);
    this._components[c][param] = v > 0 ? new Mesh3(this._engine, v) : null;
  }
  ...
  _wljs_set_component_param_material(c, p, pe, v) {
    const param = this.UTF8ViewToString(p, pe);
    this._components[c][param] = v > 0 ? new Material3(this._engine, v) : null;
  }
  _wljs_set_component_param_animation(c, p, pe, v) {
    const param = this.UTF8ViewToString(p, pe);
    this._components[c][param] = v > 0 ? new Animation3(this._engine, v) : null;
  }
  _wljs_set_component_param_skin(c, p, pe, v) {
    const param = this.UTF8ViewToString(p, pe);
    this._components[c][param] = v > 0 ? new Skin3(this._engine, v) : null;
  }
...
async function detectFeatures() {
  let [simdSupported, threadsSupported] = await Promise.all([simd3(), threads3()]);
  if (simdSupported) {
...
function checkRuntimeCompatibility(version) {
  ...
  let majorDiff = major - APIVersion3.major;
  let minorDiff = minor - APIVersion3.minor;
  ...
  if (isRuntimeOlder) {
    throw new Error(`${error}	\u2192 Please use a Wonderland Engine editor version >= ${APIVersion3.major}.${APIVersion3.minor}.*`);
  }
...
async function loadRuntime(runtime, options = {}) {
  ...
  const wasm = new WASM3(threads4);
  wasm.worker = `${filename}.worker.js`;
  wasm.wasm = wasmData;
  wasm.canvas = glCanvas;
  const engine2 = new WonderlandEngine3(wasm, loadingScreenData);
  if (!window._WL) {
    window._WL = { runtimes: {} };
  }
...
// js/absolute-scale-watcher.ts
var AbsoluteScaleWatcher = class extends Component3 {
  ARXR8SLAMCamera;
  mesh;
  material;
  _tracking = false;
  _camForward = vec3_exports.create();
  _intersectionVec3 = vec3_exports.create();
  _tmpWorldPosition = vec3_exports.create();
  start() {
    ...
    div2.innerHTML = "Move your camera back and forth<br /> until the absolute scale can be detected. <br /> Depending on the device and lightning conditions - this might take some time.";
    ...
    if (this._tracking) {
      ...
      if (this._camForward[1] < 0) {
        ...
        vec3_exports.add(
          ...
          vec3_exports.scale(this._intersectionVec3, this._camForward, t)
        );
...
__publicField(AbsoluteScaleWatcher, "TypeName", "absolute-scale-watcher");
__decorateClass([
  property3.object()
], AbsoluteScaleWatcher.prototype, "ARXR8SLAMCamera", 2);
__decorateClass([
  property3.mesh()
], AbsoluteScaleWatcher.prototype, "mesh", 2);
__decorateClass([
  property3.material()
], AbsoluteScaleWatcher.prototype, "material", 2);
...
var ButtonStartARSession = class extends Component3 {
  start() {
...
//# sourceMappingURL=AbsoluteScale-bundle.js.map
