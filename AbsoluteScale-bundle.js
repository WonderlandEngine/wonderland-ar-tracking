...
var WASM3 = class {
  ...
  tempUTF8(str2) {
    ...
    this.stringToUTF8(str2, this._tempMem, strLen);
  ...
  _wljs_component_update(component, dt) {
    ...
    if (!c) {
      ...
      this._components[component] = new Component3(this._engine);
      return;
    }
    if (!c.update)
      return;
...
async function loadRuntime(runtime, options = {}) {
  ...
  const { simd: simd4 = simdSupported, threads: threads4 = threadsSupported, physx = false, loader = false, xrFramebufferScaleFactor = 1, loadingScreen = "WonderlandRuntime-LoadingScreen.bin", canvas = "canvas" } = options;
  const variant = [];
  if (loader)
    variant.push("loader");
  if (physx)
