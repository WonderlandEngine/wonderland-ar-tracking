...
const ARSetup = {
  ...
  requires8thwall: false,

  hasNativeWorldTrackingSupport: () => WL.arSupported,
  hasNativeFaceTrackingSupport: () => false,
  hasNativeImageTrackingSupport: () => false,

  setUsage: async function (usage: ARUsage) {

    switch (usage) {
      case ARUsage.SLAM: default:
        this.requires8thwall = !this.hasNativeWorldTrackingSupport();
        break;

      case ARUsage.FACE_TRACKING:
        this.requires8thwall = !this.hasNativeFaceTrackingSupport();
        break;

      case ARUsage.IMAGE_TRACKING:
        this.requires8thwall = !this.hasNativeImageTrackingSupport();
        break;
    }

    // some component already initialized 8thwall loading
    if (this.requires8thwall) {
      await Setup8thwall.load();
    }
  ...
  startARSession: async function () {
    if (!this.ARSystem) {
      throw new Error("No provider AR was set");
    }
    await this.checkPermissions();
    this.ARSystem.startARSession();
  },

  checkPermissions: async function () {
    if (this.requires8thwall) {
      return await Setup8thwall.checkPermissions();
    }
  },

  enable8thwallCameraFeed: function () {
