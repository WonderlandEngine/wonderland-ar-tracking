...
// ../../ar-tracking/dist/src/components/AR-VPS-camera.js
var ARVPSCamera = class extends ARCamera {
  /**
  ...
  init() {
    this._trackingImpl = ARSession.getSessionForEngine(this.engine).getTrackingProvider(TrackingType.VPS, this);
  }
  start() {
    if (this._trackingImpl.init)
...
__publicField(ARVPSCamera, "TypeName", "ar-vps-camera");
...
// js/vps-example.ts
var _VPSExample = class extends Component2 {
  VPSCamera;
  waypointName;
  start() {
    if (!this.VPSCamera) {
    ...
    if (!_VPSExample._debugText) {
      _VPSExample._debugText = document.createElement("div");
      _VPSExample._debugText.id = "vps-example-debug-text";
      _VPSExample._debugText.style.fontSize = "14px";
      _VPSExample._debugText.style.position = "absolute";
      _VPSExample._debugText.style.bottom = "0";
      _VPSExample._debugText.style.left = "0";
      _VPSExample._debugText.style.color = "white";
      _VPSExample._debugText.style.textShadow = "2px 2px 4px #FFFF00";
    }
    ARSession.getSessionForEngine(this.engine).onSessionStart.add(() => {
      document.body.appendChild(_VPSExample._debugText);
      _VPSExample._debugText.innerHTML = "Looking for a waypoint";
    });
    ARSession.getSessionForEngine(this.engine).onSessionEnd.add(() => {
      _VPSExample._debugText.remove();
    });
    camera.onWaySpotFound.add(this.wayspotFound);
    camera.onWaySpotUpdated.add(this.updateModelPose);
    camera.onWaySpotLost.add(() => {
      _VPSExample._debugText.innerHTML += "<br />Way spot lost";
    });
  }
  wayspotFound = (event) => {
    if (event.name !== this.waypointName)
      return;
    _VPSExample._debugText.innerHTML += "<br />Way spot found: " + event.name;
    this.updateModelPose(event);
  };
  updateModelPose = (event) => {
    if (event.name !== this.waypointName)
      return;
    _VPSExample._debugText.innerHTML += "<br />Way spot updated: " + event.name;
...
var VPSExample = _VPSExample;
__publicField(VPSExample, "TypeName", "vps-example");
// private static toggleMeshButton: HTMLButtonElement;
__publicField(VPSExample, "_debugText");
__decorateClass([
  property2.object()
], VPSExample.prototype, "VPSCamera", 2);
__decorateClass([
  property2.string()
], VPSExample.prototype, "waypointName", 2);

// js/vps-mesh-example.ts
var VPSMeshExample = class extends Component2 {
  VPSCamera;
  generatedMeshMaterial;
  _mesh = null;
  _meshComp = null;
  _toggleMeshButton;
  start() {
  ...
  createMesh = (event) => {
    ...
    const colors = this._mesh.attribute(MeshAttribute2.Color);
    let ci = 0;
    for (let i = 0; i < colorData.length; i += 3) {
      colors.set(ci, [colorData[i], colorData[i + 1], colorData[i + 2], 1]);
      ci++;
    }
...
__publicField(VPSMeshExample, "TypeName", "vps-mesh-example");
__decorateClass([
  property2.object()
], VPSMeshExample.prototype, "VPSCamera", 2);
__decorateClass([
  property2.material()
], VPSMeshExample.prototype, "generatedMeshMaterial", 2);
...
//# sourceMappingURL=Vps-bundle.js.map
