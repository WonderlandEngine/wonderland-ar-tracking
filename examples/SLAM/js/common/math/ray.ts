...
class Ray {
    ...
    distanceSqToPoint(point: vec3) {
        const directionDistance = vec3.dot(
            vec3.sub(_vector, point, this.origin),
            this.direction
        );
        ...
        const result = vec3.add(
            _vector,
            vec3.scale(_vector, vec3.clone(this.direction), directionDistance),
            this.origin
        );
    ...
    at(t: number, target: vec3) {
        return vec3.add(
            _vector,
            vec3.scale(_vector, vec3.clone(this.direction), t),
            this.origin
        );
    }
}
